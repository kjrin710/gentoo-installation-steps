...
我没有给home和swap分区，分区这个就看个人喜好了，而且比起swap我更喜欢lz4压缩的zram。  
...
`eselect profile set X`  # 我用的是窗口管理器，所以选了default/linux/amd64/23.0/systemd (stable)，有其他需求的就选择对应的profile  
`env-update`  
...
`emerge --ask gentoo-kernel-bin` # 此命令能自动完成内核编译，主打一个方便。但这样的缺点也很明显，没有改变任何内核设置。几乎所有功能都包含在了内核里面，非常的臃肿。一般想省时省事用这个方法比较合适。  
...
**TIPS：**genkernel还有一些其他的用法，可以使用genkernel --help来查看或者去看gentoo的genkernel文档https://wiki.gentoo.org/wiki/Genkernel/zh-cn。我等下就会用到genkernel来配置initramfs  
...
若使用以上第二或第三种方法安装linux内核，可能还需要配置initramfs，可以用genkernel也可以用dracut，我选择用genkernel。  
`genkernel --install initramfs`  

使用以上三种方法的任意一种来完成内核安装操作后，一定需要配置引导加载程序。  
配置引导加载程序也有很多种，这里以grub为主，https://wiki.gentoo.org/wiki/Grub。  
要配置grub，请执行以下命令。  
